...
describe("Auth LinkedIn lead capture", () => {
	...
	it("triggers email with quick enquiry template on linkedin user creation", async () => {
		...
		expect(cfg.socialProviders.linkedin.clientId).toBe(
			MOCK_ENV.LINKEDIN_CLIENT_ID
		);
		expect(cfg.socialProviders.linkedin.clientSecret).toBe(
			MOCK_ENV.LINKEDIN_CLIENT_SECRET
		);
