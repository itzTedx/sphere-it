...
const questionScore = z
	...
	.int()
	.min(0, { message: "Please select an option for this question" })
...
export const aiMaturityUserInfoSchema = z.object({
	name: z
		...
		.refine((val) => val.length >= 2, {
			error: "Name must be at least 2 characters after trimming",
		}),
	email: z
		.email({
	...
	company: z
		...
		.transform((val) => val.trim())
		.refine((val) => val.length >= 2, {
			error: "Company name must be at least 2 characters after trimming",
		}),
	phone: z
		.string()
		.optional()
...
export type AiMaturityQuestionnaireType = z.infer<
	typeof aiMaturityQuestionnaireSchema
>;
...
export function calculateMaturityScore(
	answers: AiMaturityQuestionnaireType
): MaturityResult {
