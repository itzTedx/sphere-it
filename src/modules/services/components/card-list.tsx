...
export function Card({
	...
	if (variant === "outlined")
		return (
			...
			>
				<div
					className="h-full space-y-3 rounded-xl bg-card p-4 shadow-sm sm:p-6"
					{...props}
				/>
			</div>
		);
	...
	return (
		<CardContainer
			className={cn("h-full space-y-6 rounded-3xl p-6 shadow-md", className)}
			{...props}
		/>
	);
}
...
export function ListCardContent({ children }: { children: React.ReactNode }) {
	...
	const hasList = Children.toArray(children).some(
	...
	const transformedChildren = hasList
		? Children.map(children, (child) => {
				if (
					isValidElement<React.HTMLProps<HTMLUListElement>>(child) &&
					child.type === "ul"
				) {
					...
					return cloneElement<React.HTMLProps<HTMLUListElement>>(child, {
						...
						children: Children.map(ulProps.children, (li) => {
							if (
								isValidElement<React.HTMLProps<HTMLLIElement>>(li) &&
								li.type === "li"
							) {
								...
								return cloneElement<React.HTMLProps<HTMLLIElement>>(li, {
									...
									children: (
										<>
											<IconCheckmark
												aria-hidden="true"
												className="mt-1.5 size-4 shrink-0 text-primary-600"
											/>
	...
	return <div className="p-3 md:p-6">{transformedChildren}</div>;
}
