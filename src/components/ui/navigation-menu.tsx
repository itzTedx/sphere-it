...
function NavigationMenuTrigger({
  ...
  return (
    ...
    >
      ...
      <IconChevronDownFill
        aria-hidden="true"
        className="relative top-[1px] ml-1 size-3 text-stone-500/20 transition duration-300 group-data-[state=open]:rotate-180 group-data-[state=open]:text-stone-500/50"
      />
    </NavigationMenuPrimitive.Trigger>
  );
}
...
function NavigationMenuLink({ className, ...props }: React.ComponentProps<typeof NavigationMenuPrimitive.Link>) {
  return (
    <NavigationMenuPrimitive.Link
      className={cn(
        "flex flex-col gap-1 rounded-lg p-2 text-sm outline-none transition-all hover:bg-muted hover:text-muted-foreground focus:bg-muted focus:text-muted-foreground focus-visible:outline-1 focus-visible:ring-[3px] focus-visible:ring-ring/50 data-[active=true]:bg-muted/50 data-[active=true]:text-muted-foreground data-[active=true]:focus:bg-muted data-[active=true]:hover:bg-muted [&_svg:not([class*='size-'])]:size-4 [&_svg:not([class*='text-'])]:text-muted-foreground",
        className
      )}
