...
export function ScrollProgressRoot({
	children,
	springOptions,
	containerRef,
}: ScrollProgressRootProps) {
	...
	return (
		<ScrollProgressProvider
			value={{ progress, progressValue, progressPercentage }}
		>
			{children}
		</ScrollProgressProvider>
	);
...
export function ScrollProgressIndicator({
	...
	return (
		<NumberFlow
			className={cn(
				"w-7 font-mono text-muted-background text-xs uppercase",
				className
			)}
			suffix="%"
