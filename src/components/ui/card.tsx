...
function Card({ className, ...props }: React.ComponentProps<"div">) {
	return (
		<div
			className={cn(
				"flex flex-col overflow-hidden rounded-[calc(var(--radius-2xl)+calc(var(--spacing)*2))] border bg-card p-2 text-card-foreground shadow-sm transition-[transform,box-shadow] duration-300 ease-out hover:shadow-md",
				className
			)}
			data-slot="card"
			{...props}
...
function CardTitle({ className, ...props }: React.ComponentProps<"h3">) {
	return (
		<h3
			className={cn("font-medium text-base xl:text-title-6", className)}
...
function CardDescription({ className, ...props }: React.ComponentProps<"p">) {
	return (
		<p
			className={cn(
				"text-muted-foreground text-sm lg:text-xs xl:text-sm",
				className
			)}
