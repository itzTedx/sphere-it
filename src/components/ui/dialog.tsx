  ...
  return (
    <DialogPortalPrimitive>
      ...
      <DialogContentPrimitive
        className={cn(
          "fixed top-[50%] left-[50%] z-9999 grid w-full max-w-[calc(100%-2rem)] translate-x-[-50%] translate-y-[-50%] gap-4 rounded-lg border bg-background p-6 shadow-lg sm:max-w-lg",
          className
        )}
        {...props}
      >
...
function DialogTitle({ className, ...props }: DialogTitleProps) {
  return <DialogTitlePrimitive className={cn("text-title-3 leading-none", className)} {...props} />;
}

type DialogDescriptionProps = DialogDescriptionPrimitiveProps;

function DialogDescription({ className, ...props }: DialogDescriptionProps) {
  return <DialogDescriptionPrimitive className={cn("text-sm text-stone-500", className)} {...props} />;
}
