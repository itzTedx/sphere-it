    ...
    <li className="grid font-display lg:grid-cols-[1fr_1fr]">
      {data.map((link) => (
        <div className="flex h-full flex-col p-2" key={link.id}>
          <small className="px-2 pb-1 font-display font-medium text-sm text-stone-400 uppercase">{link.id}</small>
          <div className="grid h-full grid-cols-2 gap-2">
            {link.links.map((link) => (
              <NavigationMenuLink asChild className="group" key={link.label}>
                <Link
                  className="flex aspect-4/3 h-full w-full flex-1 select-none flex-col justify-between rounded-md border bg-stone-100 p-4 no-underline outline-hidden transition-all duration-200 hover:bg-primary-100 focus:shadow-md group-hover:border-primary-600 group-focus-visible:border-primary-600 group-focus-visible:bg-primary-100 md:p-6"
                  href={link.href}
                  title="Explore our services"
                >
