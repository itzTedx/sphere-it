			...
			<main>
					...
					<div className="container relative z-50 h-full max-w-7xl space-y-6 py-6 sm:space-y-8 sm:py-9 md:space-y-10 md:py-12 lg:py-16">
						...
						>
							...
							>
								...
								<span className="text-primary-600">Precision and Purpose.</span>
							</h1>
							<p className="text-sm text-stone-800 sm:text-base">
								At Sphere IT, we believe in empowering people who combine
								clarity of thought with the courage to innovate. Whether you're
								engineering intelligent platforms or delivering complex
								transformations, your work here shapes industries.
							</p>
						</section>
						<section
						...
						>
							<div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:col-span-7">
								<div className="flex flex-col gap-4">
									<div className="flex flex-col justify-between gap-12 rounded-2xl border bg-background px-4 py-6 sm:gap-16 sm:px-6 sm:py-9 md:gap-24">
										<p className="text-balance text-base sm:text-lg">
											Work with leading BFSI and enterprise clients on projects
											that demand accuracy, structure, and impact.
										</p>
								...
								<div className="flex flex-col gap-4">
									<div className="flex items-center justify-between gap-4 rounded-2xl border bg-background p-4 sm:p-6">
										...
										>
											{TEAMS.splice(0, 4).map((team) => (
												<Avatar
													className="transition hover:z-50 hover:scale-110"
													key={team.name}
												>
													<AvatarImage
														alt={team.name}
										...
										<Button
											aria-label="Navigate to meet our team section"
											asChild
											size="sm"
											variant="ghost"
										>
									...
									<div className="flex h-full flex-col justify-between gap-6 rounded-2xl border bg-background px-4 py-6 sm:px-6 sm:py-9">
										<p className="text-balance text-base sm:text-lg">
											We focus on real outcomes - meaningful work over
											buzzwords.
										</p>
										<div className="space-y-3">
											<IconBox>
							...
							<div className="relative flex flex-col justify-between gap-4 overflow-hidden rounded-2xl bg-foreground p-6 text-muted-background sm:p-9 lg:col-span-5">
								<div className="space-y-2">
									<h2 className="text-card text-title-4 sm:text-title-3">
										People First
									</h2>
									<p className="text-balance text-base sm:text-lg">
										Growth programs, cross-border exposure, and a culture that
										values skill, transparency, and trust.
									</p>
								</div>
				...
				>
					<header className="space-y-2 text-center sm:space-y-3">
						...
						>
							Current Openings{" "}
							<span className="text-stone-500 text-title-5 sm:text-title-4">
								({ROLES.length})
							</span>
						</h2>
					</header>
				...
				<section
					...
					className="mb-6 border-y sm:mb-9 md:mb-12 lg:mb-16 xl:mb-20"
					id="application"
				>
					<div className="container max-w-5xl">
						<div className="rounded-2xl border bg-card p-6 sm:rounded-3xl sm:p-8 md:rounded-4xl md:p-12 lg:p-16">
							...
							<h2
								className="mt-2.5 mb-4 text-primary-900 text-title-4 sm:mb-6 sm:text-title-3"
	...
	return (
		<article className="group flex h-full flex-col gap-4 rounded-2xl border bg-background p-4 transition duration-200 hover:bg-card hover:shadow-md sm:gap-6 sm:p-6">
			<div className="space-y-2 sm:space-y-3">
				<h4 className="text-stone-900 text-title-6 sm:text-title-5">
					{data.title}
				</h4>
				<div className="flex flex-wrap items-center gap-2 text-stone-700 text-xs sm:text-sm">
					...
					<span
						aria-hidden="true"
						className="size-1 rounded-full bg-stone-300"
					/>
					<div className="flex items-center gap-1.5">
						<IconStopwatch
							aria-hidden="true"
