...
import type { RoleType } from "../data/mockup-roles";
...
export function generateJobPostingStructuredData(role: RoleType, jobId: string) {
  ...
  return {
    ...
    url: `${BASE_URL}/careers/${jobId}`,
    workHours: role.time,
  };
}

export function generateBreadcrumbStructuredData(jobTitle: string, jobId: string) {
  return {
    "@context": "https://schema.org",
    "@type": "BreadcrumbList",
    itemListElement: [
      ...
      },
      {
        ...
        position: 3,
        name: jobTitle,
        item: `${BASE_URL}/careers/${jobId}`,
      },
    ],
  };
}

export function generateStructuredData(role: RoleType, jobId: string) {
  return [generateJobPostingStructuredData(role, jobId), generateBreadcrumbStructuredData(role.title, jobId)];
}
