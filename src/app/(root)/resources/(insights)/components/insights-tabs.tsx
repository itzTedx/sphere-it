    ...
    >
      <div className="relative z-50 w-full border-b bg-card">
        <TabsList
          ...
          className="h-auto w-full gap-2 rounded-none border-b bg-transparent px-0 py-1 text-foreground"
        >
          {TABS.map((tab) => {
            return (
              <TabsTrigger
                aria-controls={`${tab.link}-panel`}
                className="relative max-w-fit cursor-pointer bg-transparent font-display text-sm hover:bg-muted hover:text-foreground data-[state=active]:bg-transparent data-[state=active]:shadow-none sm:text-base"
                id={`${tab.link}-tab`}
                key={tab.link}
                value={tab.link}
              >
