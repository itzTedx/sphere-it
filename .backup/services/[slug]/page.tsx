...
import {
	...
	PreviewCardPanel,
	PreviewCardTrigger,
} from "@/components/ui/base/preview-card";
				...
				<section className="container max-w-7xl py-12">
					...
					<div className="grid gap-6 pt-9 sm:grid-cols-2 lg:grid-cols-2">
						{service.feature.features.map((feature) => (
							<Card
								className="space-y-6 rounded-3xl p-6 shadow-md"
								key={feature.title}
							>
								...
								<div className="space-y-3">
									<h3 className="text-primary-900 text-title-5">
										{feature.title}
									</h3>
									<p className="text-base text-stone-800 tracking-tight sm:text-lg">
										{feature.description}
									</p>
								</div>
							</Card>
					...
					<div className="mx-auto max-w-7xl space-y-6 py-12 max-sm:container sm:py-16 lg:py-20">
						<header className="max-w-2xl space-y-4">
							...
							<h2 className="text-primary-900 text-title-4 lg:text-title-2">
								Scale Smarter.{" "}
								<span className="text-primary-600">Grow Faster.</span> Lead with
								Data.
							</h2>
							<p className="text-lg text-muted-foreground">
								Harness the power of intelligent automation and predictive
								analytics to accelerate growth, enhance efficiency, and make
							...
							{service.transformation.features.map((feature, i) => (
								...
								>
									<div className="p-4 sm:p-6">
										<h3 className="text-primary-900 text-title-6 sm:text-title-5">
											{feature.title}
										</h3>
							...
							{service.transformation.metrics.map((metric, i) => (
								...
								>
									<div className="h-full space-y-3 rounded-xl bg-card p-4 shadow-sm sm:p-6">
										<h4 className="text-primary-800 text-title-4">
											{metric.value}
										</h4>
					...
					<div className="container max-w-7xl space-y-6 rounded-3xl border bg-card p-6 sm:space-y-9 sm:p-9">
						<header className="space-y-4">
							...
							<div className="grid gap-4 lg:grid-cols-2">
								...
								<p className="text-lg text-stone-600">
									We apply enterprise-grade AI where reliability and governance
									matter most—starting with BFSI and extending to priority
									enterprise sectors across the GCC.
								</p>
							</div>
						</header>
						<ul className="grid grid-cols-2 gap-4 lg:grid-cols-3 xl:grid-cols-5">
							{service.industries.map((industry, i) => (
								<li
									className="rounded-3xl bg-card p-4 shadow-md sm:p-6"
									key={`${i}-${industry.title}`}
								>
									<PreviewCard delay={50}>
										<PreviewCardTrigger
											aria-label={`${industry.title} sector expertise`}
										...
										<PreviewCardPanel className="w-72 p-0" side="top">
											...
											<div className="p-3">
												...
												<p className="font-display text-stone-700">
